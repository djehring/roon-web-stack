...
Completed web research and syntactically valid structured responses (including picture reviews) are checkpointed under `progress-<id>/<request-hash>/`, keyed by their full request payload. Research drafts and response checkpoints survive success as well as failure; deleting the item removes them. Rebuilds can reuse a matching successful manifest when an older bridge removed its research draft. The normal source, scene and photo validation still runs on cached results. Jobs publish a `message` describing research, archive search, downloads or picture review; persisted failures retain the failed stage and error after restart. Native polling retries temporary network errors against the same generation.
...
For GPT-5/6 models, all stages use `reasoning.effort: low`. Research allows 4,096 output tokens, JSON compilation/planning up to 6,144, and pixel review 2,048; web searches use low context and at most four tool calls per response. Other model overrides do not receive this setting. The default [GPT-5.6 Sol model supports low reasoning](https://developers.openai.com/api/docs/models/gpt-5.6-sol); no model change is required.
...
Each unchanged request has persisted reservations for at most 24 paid Responses calls, 80,000 output tokens in aggregate and 32 web-tool calls. Limits are reserved before dispatch, including concurrent work. They survive Retry and bridge restarts. Successful cached responses consume no further allowance. Timeouts and connection/service errors are never automatically resubmitted; their reservations remain because their billable usage may be unknown. The preparation deadline prevents further AI calls after ten minutes and bounds in-flight API waits to the remaining time.

`progress-<id>/<request-hash>/usage.json` records stage, model, request/finish times, response ID, returned token usage and actual search actions. Incomplete responses retain usage; requests without a returned response are marked `usage-unknown`. The ledger contains no API key or input image bytes. These are usage ceilings, not guaranteed currency limits: input/image/search-token pricing also contributes, and changing the request creates a separate allowance. Text inputs above 120,000 characters are rejected before dispatch. Existing charges cannot be reconstructed from builds whose prior bridge removed the checkpoints.

The native Retry action first reads the existing generation. It recovers a finished result or resumes polling active work before considering another update; an unreadable status does not authorize another paid submission. Regression checks use mocked OpenAI responses and archive data; the new limits have not been evaluated through a paid live rebuild.
