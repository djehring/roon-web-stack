...
test("Bowie portraits use actual archive metadata rather than claiming a different licensed studio session", () => {
  const input = validateCapsuleRequest({
    ...
    tracks: [{ artist: "David Bowie", track: "Changes", album: "Best of Bowie" }],
    options: {
      mode: "artist",
      topics: ["artistImages"],
    ...
    scenes: [
      {
        ...
        scope: "Music",
        topic: "artistImages",
        sources: [{ title: "Studio catalogue", url: "https://example.org/studio" }],
        trackIndices: [],
  ...
  const images = ["1983-06-01", "1990-03-01", "2008 (upload date)"].map((date, index) => ({
    file: `photo-${index}`,
    date,
  ...
  expect(capsule.scenes.map((scene) => scene.dateLabel)).toEqual(["1983-06-01", "1990-03-01", "2008 (upload date)"]);
  expect(capsule.scenes.map((scene) => scene.eventStart)).toEqual(["1983-06-01", "1990-03-01", undefined]);
  expect(capsule.scenes.map((scene) => scene.sources[0].url)).toEqual(images.map((image) => image.sourceUrl));
  expect(capsule.scenes.every((scene) => scene.body === "" && scene.topic === "artistImages")).toBe(true);
  expect(capsule.scenes.map((scene) => scene.images?.length)).toEqual([1, 1, 1]);
});
